...
import { Calendar, Check, ChevronDown, ChevronLeft, Clock, MapPin, Paperclip, Receipt, Settings2, Users, X } from 'lucide-react'
...
console.log("form data raw ",raw)
  ...
  if (raw.payRate) payload.payRate = Number(raw.payRate)
  if (raw.chargeRate) payload.chargeRate = Number(raw.chargeRate)
  if (raw.chargeAmount) payload.chargeAmount = Number(raw.chargeAmount)
  if (raw.requiredWorkers) payload.requiredWorkers = Number(raw.requiredWorkers)

  // Advanced options — openToClaims/requiresApproval are always present
  // (toggles default to false/true either way); clockInGraceMinutes is a
  // real number Input that's just empty when left blank, meaning "inherit".
...
export function CreateJob() {
  ...
  let { users } = useQuery(workersQuery(searchValues)).data as { users: User[] }
  ...
  const toggleWorker = (email: string) => {
    ...
    const next = exists
      ...
      : (() => {
        ...
        return worker
          ? [...selectedWorkers, { fullname: worker.fullname, email: worker.email, phone: "", user: worker._id }]
          : selectedWorkers
      })()
  ...
  const onValid = () => {
    // RHF has confirmed the data is valid; <Form> submits to the action.
  }
      ...
      >
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Priority</p>
                <RadioGroup
                  value={priority}
                  onValueChange={v => setValue("priority", v as any, { shouldValidate: true })}
                  name="priority"
                ...
                >
                  {[
                    ...
                    <Field key={item.value} orientation="horizontal">
                      <RadioGroupItem value={item.value} id={`priority-${item.value}`} />
                      <FieldContent>
                        <FieldLabel htmlFor={`priority-${item.value}`} className={cn("font-medium", item.className)}>
                          {item.label}
                        </FieldLabel>
                      </FieldContent>
                    </Field>
          ...
          {address && (
            <>
              ...
              <p className="mt-1 text-[11px] text-slate-400">
                Only the general area is shown to workers before they're assigned — the exact address is used for directions once someone is.
              </p>
            </>
          )}
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-6">
          ...
          >
            ...
            <ChevronDown size={14} className={cn("text-slate-400 transition-transform shrink-0", workerOpen && "rotate-180")} />
          </button>
          ...
          {selectedWorkers.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-3">
              {selectedWorkers.map(sw => {
                ...
                if (!w) return null
                return (
                  <div key={sw.email} className="flex items-center gap-1.5 bg-blue-50 border border-blue-200 rounded-full pl-1.5 pr-2 py-0.5">
                    <span className="text-xs font-medium text-blue-700">{w.fullname.split(' ')[0]}</span>
                    <button type="button" onClick={() => toggleWorker(sw.email)} className="text-blue-400 hover:text-blue-600">
                      <X size={11} />
                    </button>
                  </div>
                )
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-6 hidde">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 min-w-0">
            <div className="min-w-0">
              ...
              />
              <p className="text-[11px] text-slate-400 mt-1">Per hour, paid to each assigned worker</p>
          ...
          <div className="mt-5 pt-5 border-t border-[#F1F5F9] min-w-0">
            ...
            <RadioGroup
              value={chargeType}
              onValueChange={v => setValue("chargeType", v as "hourly" | "fixed", { shouldValidate: true })}
              name="chargeType"
                ...
                >
                  ...
                  >
                    <Input
                      ...
                      onChange={() => setValue("chargeType", opt.value as "hourly" | "fixed", { shouldValidate: true })}
                    />
                    <span
                      className={cn(
            ...
            {shiftHours > 0 && (payRate > 0 || totalCharge > 0) && (
              <div className="mt-4 grid grid-cols-3 gap-3 min-w-0">
                {[
                  ...
                  <div key={s.label} className="bg-slate-50 rounded-xl p-3 text-center border border-slate-100 min-w-0">
                    <p className={cn(
                      "text-sm font-bold truncate",
                      s.highlight ? (s.value >= 0 ? "text-emerald-600" : "text-red-600") : "text-slate-900"
                    )}>
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-6 hidden">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <span className="w-5 h-5 rounded-full bg-[#1E3A5F] text-white flex items-center justify-center text-[10px] font-bold shrink-0">7</span>
            <span className="flex-1 min-w-0">
              <span className="text-sm font-semibold text-slate-800 flex items-center gap-1.5">
                <Receipt size={14} className="text-slate-400 shrink-0" /> Generate an invoice for this job
              </span>
              <span className="block text-xs text-slate-400 mt-0.5">
                Creates a draft using the client and workers above — send it once the job's done
              </span>
            </span>
          </label>
              ...
              {selectedWorkers.length === 0 ? (
                ...
                <div className="border border-[#E2E8F0] rounded-xl overflow-hidden min-w-0">
                  ...
                  {selectedWorkers.map(w => {
                    ...
                    return (
                      <div key={w.email} className="grid grid-cols-[2fr_1fr_1fr_1fr] gap-3 px-4 py-2.5 border-t border-[#F1F5F9] items-center min-w-0">
                        <span className="text-sm text-slate-700 truncate">{w.fullname}</span>
                        <span className="text-sm text-right text-slate-600">{formatHours(shiftHours)}</span>
                        <Input
                          type="number"
                          step="0.01"
                          min="0"
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-6">
          <h2 className="text-sm font-semibold text-slate-800 mb-4 flex items-center gap-2">
            <span className="w-5 h-5 rounded-full bg-[#1E3A5F] text-white flex items-center justify-center text-[10px] font-bold">8</span>
