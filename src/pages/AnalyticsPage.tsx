...
function StatCard({
  ...
  return (
    <div className={`rounded-2xl border p-5 flex flex-col gap-3 ${accent ? 'bg-[var(--primary)] border-[var(--primary)]' : 'bg-card border-border'}`}>
      <div className="flex items-center justify-between">
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${accent ? 'bg-white/15 text-white' : 'bg-[var(--primary)]/8 text-[var(--primary)]'}`}>
          {icon}
        </div>
        {delta !== null && (
      ...
      <div>
        <p className={`text-2xl font-bold tracking-tight ${accent ? 'text-white' : 'text-foreground'}`}>{value}</p>
        <p className={`text-sm font-medium mt-0.5 ${accent ? 'text-white/60' : 'text-muted-foreground'}`}>{label}</p>
      </div>
      <p className={`text-xs ${accent ? 'text-white/35' : 'text-muted-foreground'}`}>{deltaLabel}</p>
    </div>
  )
}
    ...
    <div className="p-6 max-w-[1400px]">
      ...
      <div className="flex items-center justify-between mb-7 flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Performance overview for your team</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled
            className="h-9 px-4 border border-border rounded-xl text-sm font-semibold text-muted-foreground flex items-center gap-2 opacity-60 cursor-not-allowed"
          >
            <Download size={13} />
            Export
          </button>
          ...
          <div className="h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={d.hoursTrend} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ borderRadius: 12, border: '1px solid var(--border)', fontSize: 12 }} formatter={(v) => [`${v}h`, '']} />
                ...
                <Area type="monotone" dataKey="hours" name="This period" stroke="var(--primary)" strokeWidth={2} fill="url(#gradHours)" dot={false} activeDot={{ r: 4, fill: 'var(--primary)' }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={d.jobStatusBreakdown} cx="50%" cy="50%" innerRadius={52} outerRadius={76}
                  ...
                  {d.jobStatusBreakdown.map(s => <Cell key={s.status} fill={STATUS_COLORS[s.status] ?? 'var(--muted-foreground)'} />)}
                </Pie>
                <Tooltip formatter={(v) => [`${v ?? 0} shifts`, '']} contentStyle={{ borderRadius: 12, border: '1px solid var(--border)', fontSize: 12 }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
          <div className="grid grid-cols-2 gap-x-3 gap-y-2 mt-1">
            {d.jobStatusBreakdown.map(s => (
              <div key={s.status} className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full shrink-0" style={{ background: STATUS_COLORS[s.status] ?? 'var(--muted-foreground)' }} />
                <span className="text-xs text-muted-foreground">{s.label}</span>
                <span className="text-xs font-bold text-foreground ml-auto">{s.count}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-card border border-border rounded-2xl p-5">
          ...
          <div className="h-[200px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={d.workerClockInActivity} margin={{ top: 4, right: 4, left: -20, bottom: 0 }} barSize={14} barGap={4}>
                ...
                <Bar dataKey="onTime" name="On time" fill="var(--primary)" radius={[4, 4, 0, 0]} />
          ...
          <div className="flex items-center gap-5 mt-3">
            {[
              { label: 'On time', color: 'var(--primary)' },
          ...
          <div className="h-[200px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={d.regularVsOvertime} margin={{ top: 4, right: 4, left: -20, bottom: 0 }} barSize={28}>
                ...
                <Bar dataKey="overtime" name="Overtime" stackId="a" fill="var(--primary)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div className="flex items-center gap-5 mt-3">
            {[
              ...
              { label: 'Overtime', color: 'var(--primary)' },
            ].map(l => (
              <div key={l.label} className="flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-5">
          ...
          {d.topWorkers.length === 0 ? (
            <p className="text-xs text-muted-foreground py-6 text-center">No hours worked in this period yet.</p>
          ) : (
              ...
              {d.topWorkers.map((w, i) => {
                ...
                return (
                  <div key={w.workerId} className="flex items-center gap-3 py-2.5 border-b border-slate-50 last:border-0">
                    ...
                    <div className="w-8 h-8 rounded-full bg-[var(--primary)]/10 flex items-center justify-center text-[10px] font-bold text-[var(--primary)] shrink-0">
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm font-bold text-foreground">{w.hours}h</p>
                      <p className="text-xs text-muted-foreground">{w.jobs} shifts</p>
                    </div>
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-[10px] font-bold ${
                      w.completionRate >= 90 ? 'bg-emerald-50 text-emerald-700' : 'bg-muted text-muted-foreground'
                    }`}>
                      {w.completionRate}%
                    </div>
                  </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-5">
          ...
          {d.locationPerformance.length === 0 ? (
            <p className="text-xs text-muted-foreground py-6 text-center">No jobs in this period yet.</p>
          ) : (
                ...
                {d.locationPerformance.map((l, i) => (
                  <tr key={l.location} className={`border-t border-slate-50 ${i === 0 ? 'border-t-slate-100' : ''}`}>
                    <td className="py-3 flex items-center gap-2">
                      <div className="w-1.5 h-4 rounded-full bg-[var(--primary)]" style={{ opacity: Math.max(0.25, 1 - i * 0.15) }} />
                      <span className="font-medium text-foreground truncate">{l.location}</span>
                    </td>
                    <td className="py-3 text-right text-muted-foreground">{l.jobs}</td>
                    <td className="py-3 text-right text-muted-foreground">{l.hours}h</td>
                    <td className="py-3 text-right">
                      <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-bold ${
                        l.completionRate === 100 ? 'bg-emerald-50 text-emerald-700'
                        : l.completionRate >= 80 ? 'bg-blue-50 text-[var(--primary)]'
