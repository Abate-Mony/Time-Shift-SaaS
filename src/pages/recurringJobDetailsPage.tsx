...
function ScheduleStatusBadge({ active }: { active: boolean }) {
    return (
        <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${active ? 'bg-emerald-50 text-emerald-700' : 'bg-muted text-muted-foreground'
            }`}>
            ...
            <div className="bg-card rounded-2xl shadow-2xl overflow-hidden">
                <div className="px-6 pt-6 pb-5 border-b border-border flex items-start justify-between">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <StopCircle size={16} className="text-muted-foreground" />
                            <h3 className="text-base font-bold text-foreground">Stop this recurring shift?</h3>
                        </div>
                        <p className="text-sm text-muted-foreground">Choose what should happen to shifts that have already been created.</p>
                    </div>
                    <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground hover:bg-muted transition-colors ml-3 shrink-0">
                        <X size={15} />
                    </button>
                </div>
                ...
                <div className="px-6 py-5 flex flex-col gap-3">
                    ...
                    <button
                        ...
                        className={`w-full text-left p-4 rounded-xl border-2 transition-all ${choice === 'keep' ? 'border-[var(--primary)] bg-[var(--primary)]/[0.03]' : 'border-border hover:border-slate-300'
                            }`}
                    >
                        <div className="flex items-start gap-3">
                            <div className={`w-4 h-4 rounded-full border-2 mt-0.5 flex items-center justify-center shrink-0 transition-all ${choice === 'keep' ? 'border-[var(--primary)]' : 'border-slate-300'}`}>
                                {choice === 'keep' && <div className="w-2 h-2 rounded-full bg-[var(--primary)]" />}
                            </div>
                            <div>
                                <p className="text-sm font-bold text-foreground mb-0.5">Stop it repeating</p>
                                <p className="text-sm text-muted-foreground leading-relaxed">
                                    No new shifts will be created. The <strong className="text-foreground">{upcomingCount}</strong> already scheduled shifts stay as they are.
                                </p>
                            </div>
                        </div>
                    </button>
                    ...
                    <button
                        ...
                        className={`w-full text-left p-4 rounded-xl border-2 transition-all ${choice === 'cancel' ? 'border-red-400 bg-red-50/40' : 'border-border hover:border-slate-300'
                            }`}
                    >
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-sm font-bold text-foreground mb-0.5">Stop it and cancel upcoming shifts</p>
                                <p className="text-sm text-muted-foreground leading-relaxed">
                                    Stops the schedule and also cancels <strong className="text-foreground">{upcomingCount}</strong> upcoming shifts. Workers who already accepted will be notified.
                                </p>
                            </div>
                        </div>
                    </button>
                ...
                <div className="px-6 py-4 border-t border-border flex items-center justify-end gap-3">
                    <button onClick={onClose} className="h-9 px-4 text-sm font-semibold text-muted-foreground border border-border rounded-xl hover:bg-muted transition-colors">
                        Keep schedule running
                    </button>
                    <button
                        onClick={() => onConfirm(choice === 'cancel')}
                        disabled={loading}
                        className={`h-9 px-5 text-sm font-bold text-white rounded-xl transition-colors disabled:opacity-60 flex items-center gap-2 ${choice === 'cancel' ? 'bg-red-600 hover:bg-red-700' : 'bg-[var(--primary)] hover:bg-primary/90'
                            }`}
                    >
...
function RestartScheduleDialog({
        ...
        return (
            <Backdrop onClose={onClose}>
                <div className="bg-card rounded-2xl shadow-2xl p-6">
                    ...
                    <h3 className="text-base font-bold text-foreground mb-1">{"Can't restart this schedule"}</h3>
                    <p className="text-sm text-muted-foreground leading-relaxed mb-5">{blocked}</p>
                    <div className="flex gap-3">
                        <button onClick={onClose} className="flex-1 h-10 border border-border text-sm font-semibold text-muted-foreground rounded-xl hover:bg-muted transition-colors">
                            Close
                        </button>
                        <button onClick={onEditInstead} className="flex-1 h-10 bg-[var(--primary)] text-white text-sm font-bold rounded-xl hover:bg-primary/90 transition-colors">
                            Edit end date
                        </button>
                    </div>
                </div>
    ...
    return (
        <Backdrop onClose={onClose}>
            <div className="bg-card rounded-2xl shadow-2xl p-6">
                <div className="flex items-center gap-2 mb-1">
                    <PlayCircle size={16} className="text-[var(--primary)]" />
                    <h3 className="text-base font-bold text-foreground">Restart recurring shift?</h3>
                </div>
                ...
                <div className="flex items-center justify-end gap-3">
                    ...
                    <button
                        onClick={onConfirm}
                        disabled={loading}
                        className="h-9 px-5 text-sm font-bold text-white bg-[var(--primary)] rounded-xl hover:bg-primary/90 transition-colors disabled:opacity-60 flex items-center gap-2"
                    >
                        {loading && <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />}
...
function EditPatternDialog({
    ...
    return (
        <Backdrop onClose={onClose}>
            <div className="bg-card rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] w-full max-w-lg">
                <div className="px-6 pt-6 pb-5 border-b border-border flex items-start justify-between shrink-0">
                    <div>
                        <h3 className="text-base font-bold text-foreground">Edit schedule pattern</h3>
                ...
                <div className="px-6 py-5 flex flex-col gap-5 overflow-y-auto">
                    ...
                    <div className="flex flex-col gap-2">
                        <label className="text-sm font-semibold text-foreground">Repeats</label>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Input
                            ...
                            className="w-20 h-9 px-3 border border-border rounded-lg text-sm text-foreground bg-card focus:outline-none focus:ring-2 focus:ring-[var(--primary)]/15 focus:border-[var(--primary)]/40 transition-all"
                        />
                    ...
                    {/* Shift time */}
                    <div className="flex flex-col gap-2">
                        <label className="text-sm font-semibold text-foreground">Shift time</label>
                    ...
                    {form.frequency === 'weekly' && (
                        <div className="flex flex-col gap-2">
                            <label className="text-sm font-semibold text-foreground">Repeat on</label>
                    ...
                    {/* Default workers */}
                    <div className="flex flex-col gap-2">
                        <label className="text-sm font-semibold text-foreground">Default workers</label>
                        <p className="text-xs text-muted-foreground -mt-1">Automatically assigned when new shifts are generated.</p>
                        {workersLoading ? (
                            ...
                            <div className="flex flex-col gap-1 max-h-40 overflow-y-auto border border-border rounded-xl p-2">
                                {workers.map(w => (
                                    <label key={w._id} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-muted cursor-pointer">
                                        ...
                                        />
                                        <span className="text-sm text-foreground">{w.fullname}</span>
...
function SchedulePatternCard({ schedule }: { schedule: RecurringDetail }) {
    ...
    return (
        <div className="bg-card rounded-xl border border-[var(--border)] p-6 ">
            <h3 className="text-sm font-bold text-foreground mb-4">Schedule pattern</h3>
...
function DefaultWorkersCard({ workers }: { workers: { _id: string; fullname: string; email: string; profilePhoto?: FileRef | null }[] }) {
    const COLORS = ['var(--primary)', '#0D9488', '#7C3AED', '#B45309']
    return (
        <div className="bg-card rounded-xl border border-[var(--border)] p-6">
            <h3 className="text-sm font-bold text-foreground mb-1">Default workers</h3>
            <p className="text-xs text-muted-foreground mb-4">Automatically assigned when new shifts are generated.</p>
            {workers.length === 0 ? (
                <p className="text-sm text-muted-foreground italic">No default workers. New shifts are created without automatic worker assignments.</p>
            ) : (
...
function ScheduleMetadataCard({ detail }: { detail: RecurringDetail }) {
    ...
    return (
        <div className="bg-card rounded-xl border border-[var(--border)] p-5">
            <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">Schedule details</h3>
            {rows.map(r => (
...
function OccurrenceRow({
    ...
    return (
        <button
            ...
            className="w-full text-left flex items-center gap-4 px-5 py-4 hover:bg-muted/70 transition-colors border-b border-slate-50 last:border-0 group"
        >
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    <p className="text-xs font-semibold text-muted-foreground">{fmtDate(occ.date)}</p>
                    <p className="text-sm font-semibold text-foreground truncate">{occ.title}</p>
                </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
                ...
                <ChevronLeft size={14} className="rotate-180 text-slate-300 group-hover:text-muted-foreground transition-colors" />
            </div>
        </button>
    )
}
    ...
    return (
        <div className="bg-card rounded-xl border border-[var(--border)] overflow-hidden">
            ...
            <div className="px-5 pt-5 pb-0 border-b border-border">
                <h3 className="text-sm font-bold text-foreground mb-3">Generated shifts</h3>
        ...
        <div className="p-6 max-w-8xl mx-x-auto">
            ...
            <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1.5 flex-wrap">
                        <h1 className="text-xl font-bold text-foreground tracking-tight min-w-0 truncate">
                ...
                <div className="flex items-center gap-2 shrink-0">
                    <button
                        ...
                        className="h-9 px-4 text-sm font-semibold text-muted-foreground border border-border rounded-xl hover:bg-muted transition-colors flex items-center gap-2"
                    >
                    ...
                    {active ? (
                        ...
                        <button
                            ...
                            className="h-9 px-4 text-sm font-semibold text-white bg-[var(--primary)] rounded-xl hover:bg-primary/90 transition-colors flex items-center gap-2"
                        >
            ...
            <AnimatePresence>
                {!active && (
                    ...
                    >
                        <div className="flex items-start gap-3 bg-muted border border-border rounded-xl px-4 py-3.5 text-sm text-muted-foreground">
