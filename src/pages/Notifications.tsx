    ...
    <div className="p-6 max-w-2xl animate-fade-in">
      ...
      {isPending ? (
        ...
        <div className="flex flex-col gap-3">
          {notifications.map(n => {
            const { icon: Icon, bg, color } = iconMap[n.type] ?? { icon: Bell, bg: 'bg-muted', color: 'text-muted-foreground' }
            return (
              <button
                key={n._id}
                onClick={() => openNotification(n)}
                className={`flex items-start gap-4 p-4 rounded-xl border transition-colors text-left w-full ${n.isRead ? 'bg-card border-[var(--border)] hover:bg-muted/60' : 'bg-card border-blue-200 ring-1 ring-blue-100 hover:bg-blue-50/30'}`}
              >
      ...
      {data && data.totalPages > 1 && (
        <div className="flex items-center justify-center gap-1 mt-5">
          <button
            ...
            disabled={page <= 1}
            className="h-7 px-2.5 rounded-lg border border-[var(--border)] text-muted-foreground hover:bg-muted transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-card flex items-center gap-1 text-xs"
          >
