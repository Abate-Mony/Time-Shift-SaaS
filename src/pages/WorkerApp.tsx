...
// ─── Types ──────────────────────────────────────────────────────────────────
type WorkerTab = 'home' | 'jobs' | 'clock' | 'schedule' | 'profile'
    ...
    <div className="flex flex-col gap-4 pb-4">
      ...
      <div className="bg-[#1E3A5F] rounded-2xl p-5 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: 'Hours', value: `${worker.hoursThisMonth}h` },
              { label: 'Jobs', value: worker.jobsCompleted },
...
// ─── Bottom Nav ────────────────────────────────────────────────────────────────
function BottomNav({ active, onChange }: { active: WorkerTab; onChange: (t: WorkerTab) => void }) {
...
export function WorkerApp() {
  ...
  const [clockJobId, setClockJobId] = useState<string | null>(activeJob?.id ?? null)
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl shadow-slate-300/60 border border-slate-200/80 overflow-hidden flex flex-col"
            ...
            <div className="flex items-center justify-between px-6 pt-4 pb-2 bg-white shrink-0">
              ...
              <div className="flex items-center gap-1.5">
                <div className="flex gap-0.5 items-end h-3">
                  {[3, 5, 7, 9].map((h, i) => (
                    <div key={i} className={`w-1 rounded-sm ${i < 3 ? 'bg-slate-800' : 'bg-slate-300'}`} style={{ height: h }} />
                  ))}
                </div>
                <div className="w-4 h-2.5 rounded-sm border border-slate-800 relative">
                  <div className="absolute inset-[1.5px] left-[1.5px] right-[3px] bg-slate-800 rounded-sm" />
                  <div className="absolute right-[-2.5px] top-[3px] w-[2px] h-[4px] bg-slate-800 rounded-r-sm" />
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain">
              <div className="px-4 pt-2 pb-2">
                {selectedJobId && tab !== 'clock' ? (
                  <JobDetailScreen
                    jobId={selectedJobId}
                    onBack={handleBack}
                    onStartWork={() => { setClockJobId(selectedJobId); setSelectedJobId(null); setTab('clock') }}
                  />
                ) : (
                  <>
                    {tab === 'home' && (
                    ...
                    {tab === 'clock' && (
                      <ClockScreen
                        jobId={clockJobId}
                        onBack={() => setTab(selectedJobId ? 'jobs' : 'home')}
                        onDone={handleClockDone}
                      />
                    )}
                    {tab === 'schedule' && (
                      <ScheduleScreen onJobSelect={handleJobSelect} />
                    )}
            ...
            {/* Bottom nav */}
            <BottomNav active={selectedJobId && tab !== 'clock' ? tab : tab} onChange={t => { setSelectedJobId(null); setTab(t) }} />
          </div>
        </div>
      </div>
    </div>
