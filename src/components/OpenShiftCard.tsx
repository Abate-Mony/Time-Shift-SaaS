...
export default function OpenShiftCard({
  ...
  return (
    <div className="bg-card rounded-2xl border border-[var(--border)] overflow-hidden shadow-sm hover:shadow-md hover:border-slate-300 transition-all">
      <div className="h-1 bg-blue-500" />
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-foreground leading-snug">{shift.title}</p>
            <p className="text-xs text-muted-foreground mt-0.5 font-medium">{shift.client?.name}</p>
          </div>
        ...
        {(shift.instructions || shift.additional_notes) && (
          <p className="mt-3 text-xs text-muted-foreground leading-relaxed bg-muted border border-border rounded-xl px-3 py-2">
            {shift.instructions || shift.additional_notes}
          </p>
        )}
