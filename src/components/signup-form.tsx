...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    // Account exists but isn't usable yet — route to the reusable verify
    // page instead of straight into the dashboard. This is also the only
    // place that page's UI could ever show if we skipped it: redirecting
    // unmounts this whole wizard before any client-side "show step 4" state
    // change would get a chance to render.
    if (currentUser && !currentUser.isVerified) {
      return redirect(`/auth/verify-email?email=${encodeURIComponent(currentUser.email)}`)
    }
...
export function SignUpForm() {
  ...
  // On success the action redirects server-side (into the dashboard, or to
  // /auth/verify-email for an unverified account) — this component unmounts
  // either way, so there's no client-side "success" step to show here. If it
  // returns an error string instead, actionError surfaces it below.
