...
export function JobAttachmentField() {
  ...
  return (
    <div className="bg-card rounded-xl border border-[var(--border)] p-6 mb-6 min-w-0">
      <h2 className="text-sm font-semibold text-foreground mb-1">Attachment (optional)</h2>
      ...
      <div className="flex items-center gap-3 min-w-0 flex-wrap">
        ...
        {fileName && (
          <div className="flex items-center gap-1.5 min-w-0 bg-muted border border-[var(--border)] rounded-lg px-2.5 py-1.5">
            ...
            <button type="button" onClick={clear} className="text-muted-foreground hover:text-muted-foreground shrink-0">
              <X size={12} />
            </button>
          </div>
        )}
