export function SectionCard({ children, className = '' }: { children: React.ReactNode; className?: string }) {
    return (
        <div className={`bg-card border border-[var(--border)] rounded-xl p-5 ${className}`}>
            {children}
        </div>
    )
}
...
export function DataRow({ label, value }: { label: string; value?: string | null }) {
    ...
    return (
        <div className="flex items-start justify-between gap-4 py-1.5 min-w-0">
            ...
            <span className="text-xs font-semibold text-foreground text-right min-w-0 truncate">{value}</span>
        </div>
    )
}